...
/*   Updated: 2020/09/22 12:34:04 by jabenjam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		parse_resolution(t_var *var, char *line, int ac)
{
	...
	var->width = ((x > var->width || i > 10) && ac == 2 ? var->width : x);
	x = 0;
	i = 0;
	while (line && *line == ' ')
		line++;
	...
	var->height = ((x > var->height || i > 10) && ac == 2 ? var->height : x);
...
int			parse_player(t_var *var, int x, int y)
{
			...
			var->map[y][x] == 'W' || var->map[y][x] == 'E')
	{
		...
		if (!(var->player.pos_x >= 1 && var->player.pos_x <= var->size_x - 1) ||
			...
			close_game(var, "Invalid player position.\n");
	}
	return (0);
}
